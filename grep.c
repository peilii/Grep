...
static void printUsage(const char *progname) {
    ...
    fprintf(stderr,
            " -f | --with-filename       print file name with output lines\n");
    fprintf(stderr,
            " -F | --no-filename         suppress the file name prefix on "
            "output\n");
    fprintf(stderr,
    ...
    fprintf(stderr, "\n");
    fprintf(stderr,
            "PATTERN is a single string that is searched for in FILE\n");
    fprintf(stderr, "\n");
    fprintf(
        stderr,
    ...
    fprintf(
        stderr,
        "recursive, '-' otherwise.  With fewer than two FILEs, assume -F.\n");
    fprintf(stderr, "Exit status is 0 if any line is selected, 1 otherwise;\n");
}

static void printUsageShort(const char *progname) {
    fprintf(stderr, "Usage: %s [OPTION]... PATTERN [FILE]...\n", progname);
    fprintf(stderr, "Try %s --help for more information\n", progname);
}

static int grepCallback(const char *file, const char *pattern, int linenumber,
                        int filename) {
    FILE *fp = NULL;
    char *line = NULL;
    size_t linelen = 0;
    ...
    while (1) {
        ...
        if (strstr(line, pattern)) {
            if (filename) printf("%s:", file);
            if (linenumber) printf("%zu:", linenr);
            printf("%s", line);
...
static void grepDoHelper(void *pattern, void *opaque) {
    grepDoHelperData *data = opaque;

    if (data->error != 0) return;

    if (GrepDo(data->grep, pattern, data->linenumber, data->filename,
               grepCallback) < 0)
        data->error = 1;
}
